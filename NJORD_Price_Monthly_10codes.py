...
pv_share = pd.read_excel("Share_in_PV_Price.xlsx", index_col=0)
...
def NJORD_price_function(imp_summed, exp_summed, PV_mark_summed):  # Main function calculating the price model.
    ...
    for name in net_trade_df.index.values:
        ...
        for period in net_trade_df.columns.values:
            ...
            name1 = NJORD_function_test.name_cleanup(name, year) # Name1 due to weird bug where the name stayed same
            if name1 in reference.index.values:
                output_P_each_year, output_P_MF_each_year = create_output_price(reference, str(year), month, name1,
                                                                                installed_capacity,
...
output_P_MF_each_year.to_excel(path_output+"NJORD-Price_model_results_2022.xlsx")
