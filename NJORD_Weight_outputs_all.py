...
unit = "Weight"
#### change path also in line 331-332 for not maximized models ######
...
for year in period:
    for name in nations_list:
        ...
        #Calculaiton of the Nemarket_factort Trade
        ...
        output_W_each_year.at[name, "Ref " + year_test] = ref_value
        output_W_each_year.at[name, "Source " + year_test] = source
        output_W_each_year.at[name, "IRENA " + year_test] = reference_data_year[Irena][name]
        output_W_each_year.at[name, "IRENA s " + year_test] = reference_data_year[str(year_test)+" - IRENA s"][name]
        output_W_each_year.at[name, "PVPS " + year_test] = reference_data_year[PVPS][name]
        output_W_each_year.at[name, "Other " + year_test] = reference_data_year[other][name]

output_W_each_year.to_excel(path_output+"Weight_max_model_results.xlsx")
output_W_each_year.to_excel(path_output+"NJORD-Weight_model_results.xlsx")


#### This script calculate the installed capacity
unit = "Weight"
path_input = "C:\\Users\\lucar\\PycharmProjects\\NJORD_2022_Albin\\Raw_data\\Final_database\\before_maximizing_bk\\Weight\\"  # this is the path_out_final in the script From_html_to_db
path_output = "C:\\Users\\lucar\\PycharmProjects\\NJORD_2022_Albin\\"# this will be the folder from where the GUI will read the data
os.makedirs(path_output, exist_ok=True)
nations_list = os.listdir(path_input + "\\Export\\")
